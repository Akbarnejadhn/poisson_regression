...
The model shows that the log rate ratio of the number of satellites residing near female crabs is `r round(m1_summary$coefficients[2], 3)` per one unit increase in female crab's carapace width. The p-value for this covariate is 2*10^-16 which shows this covariate is highly significant in the model.
...
### 1-b
In this part I am going to fit a model using both **weight** and **carapace width**:
...
The model shows that the log rate ratio of the number of satellites residing near female crabs is `r round(m2_summary$coefficients[2], 3)` per one unit increase in female crab's carapace width, holding their weigt constant. The p-value for this covariate is `r round(m2_summary$coefficients[11], 3)` which shows this covariate is not significant in the model.
...
The above plot shows that there is probably a source of overdispersion in the model because $\phi\neq 1$. Now, I will calculate the dispersion paarmeter and refit the model taking this parameter into account.
...
The dispersion parameter is `r round(phi, 3)` for M2. After adjusting for over dispersion, it can be observed that the coefficients do not differ that much. However the covariate **weight** which was significant before adjusting for overdispersion becomes insignificant after adjusting for overdispersion. We can also see that the overalll rsults are very similar using negative binomial model and the coefficients only slightly differ.
